...
const Sidebar = () => {
  ...
  const hasPermission = (resource) => {
    return userData?.permissions?.[resource]?.includes("read");
  };

  // Define all possible links with corresponding backend resource keys
  const links = [
    { to: "/", label: "Dashboard", icon: <Home size={18} />, resource: null },
    { to: "/customers", label: "Customers", icon: <Users size={18} />, resource: "customers" },
    { to: "/items", label: "Items", icon: <Package size={18} />, resource: "items" },
    { to: "/orders", label: "Orders", icon: <ShoppingCart size={18} />, resource: "orders" },
    { to: "/invoices", label: "Invoices", icon: <FileText size={18} />, resource: "invoices" },
    { to: "/payments", label: "Payments", icon: <CreditCard size={18} />, resource: "payments" },
    { to: "/users", label: "User", icon: <User size={18} />, resource: "users" },
    { to: "/settings", label: "Settings", icon: <Settings size={18} />, resource: "settings" },
  ];

  // Filter links based on user permissions
  const visibleLinks = links.filter(link => !link.resource || hasPermission(link.resource));
          ...
          <nav className="flex flex-col space-y-2">
            {visibleLinks.map(({ to, label, icon }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 px-3 py-2 rounded-md transition
                  ${isActive ? "bg-indigo-600 text-white shadow-md" : "hover:bg-slate-800 hover:text-indigo-300"}
                  ${isCollapsed ? "md:justify-center" : ""}`
                }
                onClick={() => setIsOpen(false)}
                title={isCollapsed ? label : ""}
